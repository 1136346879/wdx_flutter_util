...
class HYDateUtil {
  ...
  static String? getDateStrByTimeStr(
      String dateStr, {
        HYDateFormat format = HYDateFormat.NORMAL,
        String? dateSeparate,
        String? timeSeparate,
        bool? isUtc,
      }) {
  ...
  static String formatZHDateTime(
      String time, HYDateFormat format, String? timeSeparate) {
    time = convertToZHDateTimeString(time, timeSeparate);
    switch (format) {
      case HYDateFormat.ZH_NORMAL: //yyyy年MM月dd日 HH时mm分ss秒
        time = time.substring(
            0,
            "yyyy年MM月dd日 HH时mm分ss秒".length -
                (timeSeparate == null || timeSeparate.isEmpty ? 0 : 1));
        break;
      case HYDateFormat.ZH_YEAR_MONTH_DAY_HOUR_MINUTE: //yyyy年MM月dd日 HH时mm分
        time = time.substring(
            0,
            "yyyy年MM月dd日 HH时mm分".length -
                (timeSeparate == null || timeSeparate.isEmpty ? 0 : 1));
        break;
      case HYDateFormat.ZH_YEAR_MONTH_DAY: //yyyy年MM月dd日
        time = time.substring(0, "yyyy年MM月dd日".length);
        break;
      case HYDateFormat.ZH_YEAR_MONTH: //yyyy年MM月
        time = time.substring(0, "yyyy年MM月".length);
        break;
      case HYDateFormat.ZH_MONTH_DAY: //MM月dd日
        time = time.substring("yyyy年".length, "yyyy年MM月dd日".length);
        break;
      case HYDateFormat.ZH_MONTH_DAY_HOUR_MINUTE: //MM月dd日 HH时mm分
        time = time.substring(
            "yyyy年".length,
            "yyyy年MM月dd日 HH时mm分".length -
                (timeSeparate == null || timeSeparate.isEmpty ? 0 : 1));
        break;
      case HYDateFormat.ZH_HOUR_MINUTE_SECOND: //HH时mm分ss秒
        time = time.substring(
            "yyyy年MM月dd日 ".length,
            "yyyy年MM月dd日 HH时mm分ss秒".length -
                (timeSeparate == null || timeSeparate.isEmpty ? 0 : 1));
        break;
      case HYDateFormat.ZH_HOUR_MINUTE: //HH时mm分
        time = time.substring(
            "yyyy年MM月dd日 ".length,
            "yyyy年MM月dd日 HH时mm分".length -
                (timeSeparate == null || timeSeparate.isEmpty ? 0 : 1));
  ...
  /// dateSeparate    date separate.
  /// timeSeparate    time separate.
  static String formatDateTime(String time, HYDateFormat format,
      String? dateSeparate, String? timeSeparate) {
    switch (format) {
      case HYDateFormat.NORMAL: //yyyy-MM-dd HH:mm:ss
        time = time.substring(0, "yyyy-MM-dd HH:mm:ss".length);
        break;
      case HYDateFormat.YEAR_MONTH_DAY_HOUR_MINUTE: //yyyy-MM-dd HH:mm
        time = time.substring(0, "yyyy-MM-dd HH:mm".length);
        break;
      case HYDateFormat.YEAR_MONTH_DAY: //yyyy-MM-dd
        time = time.substring(0, "yyyy-MM-dd".length);
        break;
      case HYDateFormat.YEAR_MONTH: //yyyy-MM
        time = time.substring(0, "yyyy-MM".length);
        break;
      case HYDateFormat.MONTH_DAY: //MM-dd
        time = time.substring("yyyy-".length, "yyyy-MM-dd".length);
        break;
      case HYDateFormat.MONTH_DAY_HOUR_MINUTE: //MM-dd HH:mm
        time = time.substring("yyyy-".length, "yyyy-MM-dd HH:mm".length);
        break;
      case HYDateFormat.HOUR_MINUTE_SECOND: //HH:mm:ss
        time =
            time.substring("yyyy-MM-dd ".length, "yyyy-MM-dd HH:mm:ss".length);
        break;
      case HYDateFormat.HOUR_MINUTE: //HH:mm
  ...
  static bool isZHFormat(HYDateFormat format) {
    return format == HYDateFormat.ZH_DEFAULT ||
        format == HYDateFormat.ZH_NORMAL ||
        format == HYDateFormat.ZH_YEAR_MONTH_DAY_HOUR_MINUTE ||
        format == HYDateFormat.ZH_YEAR_MONTH_DAY ||
        format == HYDateFormat.ZH_YEAR_MONTH ||
        format == HYDateFormat.ZH_MONTH_DAY ||
        format == HYDateFormat.ZH_MONTH_DAY_HOUR_MINUTE ||
        format == HYDateFormat.ZH_HOUR_MINUTE_SECOND ||
        format == HYDateFormat.ZH_HOUR_MINUTE;
  }
  ...
  static String formatDate(DateTime? dateTime, {bool? isUtc, String? format}) {
    ...
    format = format ?? HYDataFormats.full;
  ...
  static String getFormatDate(DateTime? dateTime, {bool isUtc = false, String format = 'yyyy-MM'}) {
   return formatDate(dateTime,isUtc: isUtc,format:format);
  }
  ...
  /// [year]
  /// [month]
  static DateTime getNextMonthTime(int year, int month,){
  ...
  /// [year]
  /// [month]
  static DateTime getLastMonthTime(int year, int month,){
    int newYear, newMonth;
    if (month == 1){
      newMonth = 12;
      newYear = year - 1;
  ...
  /// 上午下午显示
  static String getZHMorningAfternoonForDateTime(DateTime dateTime){
  ...
  // 两日期相差多少天
  static int differenceDays(DateTime startTime, DateTime endTime) {
